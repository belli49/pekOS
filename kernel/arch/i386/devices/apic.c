...
void init_apic() {
  ...
  */
  ...
  // problem is here it becomes 0x0000FEC0 if we do "io_apic* io_apic1 = (io_apic*) (io_apic1_head + 1);"
  // for some reason, not adding the 1 worked though
  io_apic* io_apic1 = (io_apic*) io_apic1_head;
  

  // map ioapic address
  uintptr_t ioapic_phys_address = io_apic1->io_apic_address;
  uintptr_t ioapic_virt_address = (((uintptr_t) find_free_virtaddr()) & 0xFFFFF000) | (ioapic_phys_address & 0xFFF);
  map_page((uintptr_t*) (ioapic_phys_address & 0xFFFFF000), (uintptr_t*) (ioapic_virt_address & 0xFFFFF000), 3);
  ...
  for (uint32_t i = 0; i < 16; i++) {
    ...
    write_ioapic_register((uintptr_t) ioapic_virt_address, 0x11 + (i * 2), upper_reg);
  }


  printf("%x\n", read_ioapic_register((uintptr_t) ioapic_virt_address, 0x0));
  printf("%x\n", read_ioapic_register((uintptr_t) ioapic_virt_address, 0x1));
  printf("%x\n", read_ioapic_register((uintptr_t) ioapic_virt_address, 0x10));
  printf("%x\n", read_ioapic_register((uintptr_t) ioapic_virt_address, 0x12));
}
