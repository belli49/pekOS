...
void init_fadt() {
  ...
  if (!rsdp->revision || fadt->BootArchitectureFlags & 2) {
    ...
    if (is_dual_channel) {
      ...
      outb(PS2_COMMAND_REGISTER, 0x20);
      is_dual_channel = !(inb(PS2_DATA_PORT) & 0x20); // if bit is clear it is dual channel
    ...
    first_controller_works = !inb(PS2_DATA_PORT);
    if (!first_controller_works) printf("Interface test failed for port 1\n");
    // second port
    outb(PS2_COMMAND_REGISTER, 0xA9);
    second_controller_works = !inb(PS2_DATA_PORT);
    if (!second_controller_works) printf("Interface test failed for port 2\n");


    // enable controllers
    if (first_controller_works) outb(PS2_COMMAND_REGISTER, 0xAE);
    if (second_controller_works) outb(PS2_COMMAND_REGISTER, 0xA8);
